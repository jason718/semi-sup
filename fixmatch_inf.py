...
class FixMatch(CTAReMixMatch):
    ...
    def compute_influence(self, grad_vec_wrt_val_loss, x, y, w_match):
        ...
        jacobian_per_ex_u = jacobian_per_ex_u.reshape(FLAGS.batch*FLAGS.uratio, -1).transpose()
    ...
    def train_step(self, train_session, gen_labeled, gen_unlabeled, ep=0):       
        ...
        if ep > self.inf_warm and self.tmp.step % (self.inner_steps * FLAGS.batch) == 0:
            ...
            influences = self.compute_influence(grad_vec_wrt_val_loss, x, y, w_match)
            self.update_influence(batch_ids, influences)
    ...
    def grad_wrt_val_loss_batch(self, batch=None, feed_extra=None, **kwargs):
        ...
        indexes = np.random.randint(low=0, high=images.shape[0], size=batch*self.params['uratio'])
    ...
    def train(self, train_nimg, report_nimg):
        ...
        with tf.train.MonitoredTrainingSession(
            ...
            while self.tmp.step < train_nimg:
                ...
                for _ in loop:
                    self.train_step(train_session, gen_labeled, gen_unlabeled, ep)
    ...
    def model(self, batch, lr, wd, wu, confidence, uratio, ema=0.999, **kwargs):
        ...
        w_match = tf.placeholder(tf.float32, [batch * uratio], 'w_match') # weights for unlabeled data
        ...
        loss_xe = tf.nn.softmax_cross_entropy_with_logits_v2(labels=l_in_1hot, logits=logits_x)
        ...
        pseudo_labels_hard = tf.one_hot(tf.argmax(pseudo_labels, axis=1), kwargs['nclass'])
        loss_xeu_all = tf.nn.softmax_cross_entropy_with_logits_v2(labels=pseudo_labels_hard, logits=logits_strong)
    ...
    def model_val(self, batch, uratio, **kwargs):
        hwc = [self.dataset.height, self.dataset.width, self.dataset.colors]
        x = tf.placeholder(tf.float32, [batch * uratio] + hwc, 'v')  # validation labeled
        l_in = tf.placeholder(tf.int32, [batch * uratio], 'labels')  # Labels
        classifier = lambda x, **kw: self.classifier(x, **kw, **kwargs).logits
        logits = utils.para_cat(lambda x: classifier(x, training=True), x)
        loss_xe = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=l_in, logits=logits)) # loss
    ...
    def model_per_ex(self, nclass, batch, confidence, uratio, **kwargs):
        ...
        logits = classifier(x, training=True) #utils.para_cat(lambda x: classifier(x, training=True), x)
        logits_weak, logits_strong = tf.split(logits, 2)
        # Pseudo-label cross entropy for unlabeled data
        pseudo_labels = tf.stop_gradient(tf.nn.softmax(logits_weak))
        pseudo_labels_hard = tf.one_hot(tf.argmax(pseudo_labels, axis=1), nclass)
        loss_xeu_all = tf.nn.softmax_cross_entropy_with_logits_v2(labels=pseudo_labels_hard, logits=logits_strong)
        pseudo_mask = tf.to_float(tf.reduce_max(pseudo_labels, axis=1) >= confidence)
        loss_xeu = tf.reduce_mean(loss_xeu_all * pseudo_mask * w_match)
        # per-ex-grad_wrt_unlabeled_loss
        grads_train_per_ex = custom_gradient(loss_xeu_all, self.model_params)
        # jacobian_per_ex = [value for _, value in grads_train_per_example.items()][0]
        return utils.EasyDict(y=y_in, w_match=w_match, grads_train_per_ex=grads_train_per_ex)

    def update_influence(self, batch_ids, grad, beta_1=0.9, beta_2=0.999, epsilon=1e-8):
