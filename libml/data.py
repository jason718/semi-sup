...
class DataSets:
    def __init__(self, name, train_labeled: DataSet, train_unlabeled: DataSet, test: DataSet, valid: DataSet,
        ...
        self.size_unlabeled = size_unlabeled

    @classmethod
    def creator(cls, name, seed, label, valid, augment, parse_fn=record_parse, do_memoize=True,
        ...
        def create():
            ...
            train_unlabeled_no_valid = train_unlabeled.skip(valid).memoize()
            size_unlabeled = train_unlabeled_no_valid.get_size()
            ...
            return cls(name + '.' + FLAGS.augment + fullname + '-' + str(valid)
                       ...
                       train_unlabeled=train_unlabeled_no_valid, # train_unlabeled.skip(valid),
...
def create_datasets(augment_fn):
    ...
    d.update([DataSets.creator('cifar10', seed, label, valid, augment_fn)
              for seed, label, valid in itertools.product(range(6), [10 * x for x in SAMPLES_PER_CLASS], [1, 1000, 5000])])
