          ...
          with ops.name_scope(op.name + "_grad"):
            ...
            with src_graph._original_op(op):
              ...
              if grad_fn:
                ...
                # out_dict = PerExampleGradsForParametersVmap(op, out_grads, out_dict, in_grads)
                out_dict = PerExampleGradsForParametersVmap_fc_bn(op, out_grads, out_dict)
              else:
...
def PerExampleGradsForParameters(op, out_grads, out_dict):
  batch_size = tf.shape(op.inputs[0])[0]
  if "Conv2D" in op.name:
    def loop_fn(i):
      return gen_nn_ops.conv2d_backprop_filter(
        array_ops.expand_dims(array_ops.gather(op.inputs[0], i), 0),
        array_ops.shape_n([op.inputs[1]])[0],
        array_ops.expand_dims(array_ops.gather(out_grads[0], i), 0),
    ...
    out_dict[op.inputs[1].name[:-6]+'grad'] = cfp.pfor(loop_fn, batch_size)
  elif "BiasAdd" in op.name:
    try:
      data_format = op.get_attr("data_format")
    except ValueError:
      data_format = None
    ...
    # out_dict[op.inputs[1].name] = out_grads[0] #array_ops.shape_n([op.inputs[1]])
    t_a = op.get_attr("transpose_a")
    t_b = op.get_attr("transpose_b")
    _a = math_ops.conj(op.inputs[0])
    def loop_fn(i):
      a = array_ops.expand_dims(array_ops.gather(_a, i), 0)
      b = array_ops.expand_dims(array_ops.gather(out_grads[0], i), 0)
      if not t_a and not t_b:
        return gen_math_ops.mat_mul(a, b, transpose_a=True)
      elif not t_a and t_b:
        return gen_math_ops.mat_mul(b, a, transpose_a=True)
      elif t_a and not t_b:
        return gen_math_ops.mat_mul(a, b)
      elif t_a and t_b:
        return gen_math_ops.mat_mul(b, a, transpose_a=True, transpose_b=True)
    out_dict[op.inputs[1].name[:-6]+'grad'] = cfp.pfor(loop_fn, batch_size)
  elif "FusedBatchNorm" in op.name:
    data_format = op.get_attr("data_format")
    ...
    # grad_y = out_grads[0]
    # x = op.inputs[0]
    # grad_x, _grad_scale, _grad_offset = _BatchNormGrad(
    #     grad_y, x, scale, pop_mean, pop_var, epsilon, data_format, is_training)
    # out_dict[op.inputs[1].name[:-6]+'grad_x'] = grad_x
    # out_dict[op.inputs[1].name[:-6]+'grad_scale'] = _grad_scale
    # out_dict[op.inputs[1].name[:-6]+'grad_offset'] = _grad_offset

    def loop_fn(i):
      _, grad_scale_i, grad_offset_i = _BatchNormGrad(
        array_ops.expand_dims(array_ops.gather(out_grads[0], i), 0),
        array_ops.expand_dims(array_ops.gather(op.inputs[0], i), 0),
        scale, pop_mean, pop_var, epsilon, data_format, is_training=is_training)
      return grad_scale_i, grad_offset_i
    grad_scale, grad_offset = cfp.pfor(loop_fn, batch_size)
    out_dict[op.inputs[1].name[:-6]+'grad'] = grad_scale
    out_dict[op.inputs[2].name[:-6]+'grad'] = grad_offset 
  return out_dict


def PerExampleGradsForParametersVmap(op, out_grads, out_dict, in_grads):
  scope_name = op.name.split('/')[0]
  if "Conv2D" in op.name:
    def loop_fn(args):
      x, grad_y = args
      return gen_nn_ops.conv2d_backprop_filter(
        ...
        use_cudnn_on_gpu=op.get_attr("use_cudnn_on_gpu"),
        data_format=op.get_attr("data_format"))
    op_name = op.inputs[1].name[:-6]
    if op_name not in out_dict.keys():
      out_dict[op_name] = {scope_name: tf.vectorized_map(loop_fn, (op.inputs[0], out_grads[0])) }
    else:
      out_dict[op_name][scope_name] = tf.vectorized_map(loop_fn, (op.inputs[0], out_grads[0]))
  elif "BiasAdd" in op.name:
    try:
    ...
    bias_grad = tf.dtypes.cast(tf.vectorized_map(loop_fn, out_g), dtype=tf.float32)
    op_name = op.inputs[1].name[:-6]
    if op_name not in out_dict.keys():
      out_dict[op_name] = {scope_name: bias_grad}
    else:
      out_dict[op_name][scope_name] = bias_grad
    # out_dict[op.inputs[1].name[:-6]+'grad'] = tf.vectorized_map(loop_fn, out_grads[0])
    # out_dict[op.name] = tf.vectorized_map(loop_fn, out_grads[0])
    ...
    def loop_fn(args):
      ...
      if not t_a and not t_b:
        ...
        return gen_math_ops.mat_mul(b, a, transpose_a=True, transpose_b=True)
    # out_dict[op.inputs[1].name[:-6]+'grad'] = tf.vectorized_map(loop_fn, (_a, out_grads[0]))
    # out_dict[op.name] = tf.vectorized_map(loop_fn, (_a, out_grads[0]))
    op_name = op.inputs[1].name[:-6]
    if op_name not in out_dict.keys():
      out_dict[op_name] = {scope_name: tf.vectorized_map(loop_fn, (_a, out_grads[0]))}
    else:
      out_dict[op_name][scope_name] = tf.vectorized_map(loop_fn, (_a, out_grads[0]))
  elif "FusedBatchNorm" in op.name:
    scale = op.inputs[1]
    ...
    def loop_fn(args):
      ...
      if is_training:
        args = {
          "y_backprop":  tf.expand_dims(grad_y, 0),
    ...
    _, grad_scale, grad_offset, _, _ = tf.vectorized_map(loop_fn, (op.inputs[0], out_grads[0]))
    
    op_name1 = op.inputs[1].name[:-6]
    op_name2 = op.inputs[2].name[:-6]
    if op_name2 not in out_dict.keys():
      out_dict[op_name2] = {scope_name: grad_offset }
    else:
      out_dict[op_name2][scope_name] = grad_offset

    if op_name1 not in out_dict.keys():
      out_dict[op_name1] = {scope_name: grad_scale }
    else:
      out_dict[op_name1][scope_name] = grad_scale
  return out_dict


def PerExampleGradsForParametersVmap_fc_bn(op, out_grads, out_dict):
  ...
  #   epsilon = op.get_attr("epsilon")
  #   is_training = op.get_attr("is_training")
  #   scale = tf.dtypes.cast(op.inputs[1], tf.float64)
  #   pop_mean = tf.dtypes.cast(op.inputs[3], tf.float64)
  #   pop_var = tf.dtypes.cast(op.inputs[4], tf.float64)
  #   def loop_fn(args):
  #     x, grad_y = args
  #     _, grad_scale_i, grad_offset_i = _BatchNormGrad(
  #       tf.expand_dims(grad_y, 0),
  ...
  #       scale, pop_mean, pop_var, epsilon, data_format, is_training=is_training)
  #     return grad_scale_i, grad_offset_i
  #   inp_x = tf.dtypes.cast(op.inputs[0], tf.float64)
  #   out_g = tf.dtypes.cast(out_grads[0], tf.float64)
  #   grad_scale, grad_offset = tf.vectorized_map(loop_fn, (inp_x, out_g))
  #   out_dict[op.inputs[1].name[:-6]+'grad'] = grad_scale
  #   out_dict[op.inputs[2].name[:-6]+'grad'] = grad_offset 
...
def _BatchNormGrad(grad_y,
  ...
  """Returns the gradients for the 3 inputs of BatchNorm.
    ...
    "blob/7b32b175fe7e87766dce28749c16236111bea6a9/" +
    "tensorflow/python/ops/nn_grad.py#L929"
  """
  ...
  if is_training:
    ...
    if batch_var is not None:
      ...
      var_x = math_ops.reduce_mean(
          math_ops.squared_difference(x, tf.stop_gradient(mean_x)),
          reduce_axis, keepdims=keepdims)
